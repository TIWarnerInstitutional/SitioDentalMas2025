    ...
    <section className="max-w-5xl mx-auto pt-8 pb-8 px-4">
      <div className="bg-red-600 rounded-2xl p-8 flex flex-col md:flex-row gap-8 items-center">
        <div className="flex-1 w-full">
          ...
          {filtered.length > 3 && (
            <button
              className="w-full bg-white text-red-600 font-semibold rounded p-3 hover:bg-red-50 transition"
              onClick={() => setShowAll((s) => !s)}
            >
              {showAll ? "Mostrar menos" : "Ver todas nuestras clínicas"}
            </button>
          )}
        </div>
      ...
      {filtered.length > 3 && (
        <div className="max-w-5xl mx-auto px-4 mb-8 text-center mt-6">
          <button
            onClick={() => setShowAll((s) => !s)}
