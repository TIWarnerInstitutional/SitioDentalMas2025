...
export default function CitaSucursalPage() {
  ...
  useEffect(() => {
    ...
    const sucursal = sucursales.find(s => {
      const slug = s.nombre
        .toLowerCase()
        .replace(/dental más /gi, '')
        .replace(/\s+/g, '-')
        .replace(/[^a-z0-9-]/g, '');
      return slug === sucursalSlug.toLowerCase();
    });

    if (sucursal && sucursal.agenda) {
      ...
      // Redirigir al link de agenda después de 500ms para dar tiempo al tracking
      setTimeout(() => {
        window.location.href = sucursal.agenda || '/';
      }, 500);
    } else {
